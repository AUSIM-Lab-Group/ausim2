...
RuntimeOutput VehicleRuntime::Step(
    const RuntimeInput& input,
    bool recompute_control,
    double control_dt) {
  if (recompute_control) {
    ...
    cached_command_ =
        controller_.controlUpdate(input.current_state, cached_goal_.state, control_dt, cached_goal_.forward);
  }
