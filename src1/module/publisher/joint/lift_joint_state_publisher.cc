...
#include "./lift_joint_state_publisher.h"
...
template <class T>
LiftJointStatePublisher<T>::LiftJointStatePublisher() : BasePublisher() {}

template <class T>
bool LiftJointStatePublisher<T>::Init() {
...
template <class T>
void LiftJointStatePublisher<T>::Publish() {
  if (!publisher_) {
    return;
  }
  ...
  sim::mj::GetLiftJointState(state);  // 模拟器API
  ...
  JOINT_TOOLS_PRINT_N_INFO(1, "Publish Lift Joint Position: {}", sim::joint::tools::GetNamePosition(state));

  if (IsPrintable()) {
    AIMRT_INFO("Publish Lift Joint Position: {}", sim::joint::tools::GetNamePosition(state));
  }
}
