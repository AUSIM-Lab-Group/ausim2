...
#include "./arm_joint_state_publisher.h"
...
template <class T>
ArmJointStatePublisher<T>::ArmJointStatePublisher() : BasePublisher() {}

template <class T>
bool ArmJointStatePublisher<T>::Init() {
...
template <class T>
void ArmJointStatePublisher<T>::Publish() {
  if (!publisher_) {
    return;
  }
  ...
  // sim::api::GetArmJointState(state);  // SIM API
  sim::mj::GetArmJointState(state);  // 模拟器API
  StateType msg;
  ...
  JOINT_TOOLS_PRINT_N_INFO(1, "Publish Arm Joint Position: {}", sim::joint::tools::GetNamePosition(state));

  if (IsPrintable()) {
    AIMRT_INFO("Publish Arm Joint Position: {}", sim::joint::tools::GetNamePosition(state));
  }
}
