...
#include "./head_joint_state_publisher.h"
...
template <class T>
HeadJointStatePublisher<T>::HeadJointStatePublisher() : BasePublisher() {}

template <class T>
bool HeadJointStatePublisher<T>::Init() {
...
template <class T>
void HeadJointStatePublisher<T>::Publish() {
  if (!publisher_) {
    return;
  }
  ...
  JOINT_TOOLS_PRINT_N_INFO(1, "Publish Head Joint Position: {}", sim::joint::tools::GetNamePosition(state));

  if (IsPrintable()) {
    AIMRT_INFO("Publish Head Joint Position: {}", sim::joint::tools::GetNamePosition(state));
  }
}
