...
- 手柄输入状态：收到 `/joy` 后显示全部 `axes[]` 和 `buttons[]`；轴在左侧显示为“编号在上、竖直状态条居中、数值在下”，按钮在右侧显示为“编号在上、红/绿圆点居中”并自适应换行，不显示额外状态标题。
- 手柄到 `cmd_vel` 映射：可折叠标题栏下在线调整 `linear.x/y/z` 和 `angular.z` 对应的手柄轴、轴输出反向和手柄/键盘同步最大输出；保存时写回 `axes.*`、`scale.*` 与 `keyboard.scale.*`。
- 窗口尺寸与折叠状态：点击“应用并保存”会写回当前 GUI 窗口宽高，以及 `cmd_vel` 映射区和 action 映射区的展开/折叠状态，下次启动时加载。
