...
- 顶部输入窗格：输入来源和键盘操作提示固定显示，不随下方折叠窗格展开/收起而移动到折叠内容中。
- 手柄状态显示：可折叠标题栏下收到 `/joy` 后显示全部 `axes[]` 和 `buttons[]`；轴在左侧显示为“编号在上、竖直状态条居中、数值在下”，按钮在右侧显示为“编号在上、红/绿圆点居中”并自适应换行。
- 手柄到 `cmd_vel` 映射：可折叠标题栏下在线调整 `linear.x/y/z` 和 `angular.z` 对应的手柄轴、轴输出反向和手柄/键盘同步最大输出；保存时写回 `axes.*`、`scale.*` 与 `keyboard.scale.*`。
- 窗口尺寸与折叠状态：点击“应用并保存”会写回当前 GUI 窗口宽高，以及手柄状态区、`cmd_vel` 映射区、action 映射区和 action 历史区的展开/折叠状态，下次启动时加载。
- 语言切换：点击 `English` / `中文` 即时切换界面提示文字，保存时写回 `gui.language`。
- 手柄到 action 映射：可折叠标题栏下默认显示 `action1..action6` 6 个槽位，可编辑 service、按钮组合和键盘快捷键；“捕获”会读取当前按下的手柄按钮。
- Action 历史：可折叠显示完整历史；折叠时标题栏显示最后一条 action 的名称和接收状态，不显示时间戳。
