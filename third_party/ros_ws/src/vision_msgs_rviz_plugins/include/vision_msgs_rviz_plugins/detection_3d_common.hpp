...
#include <ausim_msg/msg/bounding_box3_d.hpp>
#include <ausim_msg/msg/detection3_d.hpp>
...
public:
  ...
  using Detection3DArray = ausim_msg::msg::Detection3DArray;
...
protected:
  ...
  visualization_msgs::msg::Marker::SharedPtr get_marker(
    const ausim_msg::msg::BoundingBox3D & box)
  {
  ...
  void showBoxes(
    const ausim_msg::msg::Detection3DArray::ConstSharedPtr & msg,
  ...
  void showBoxes(const ausim_msg::msg::Detection3D::ConstSharedPtr & msg, const bool show_score)
  {
    edges_.clear();
    m_marker_common->clearMarkers();
    ClearScores(show_score);
    ...
    const bool show_score)
  {
    ...
    for (size_t idx = 0; idx < msg->detections.size(); idx++) {
      ausim_msg::msg::BoundingBox3D box = msg->detections[idx].bbox;
  ...
  void showEdges(const ausim_msg::msg::Detection3D::ConstSharedPtr & msg, const bool show_score)
  {
    m_marker_common->clearMarkers();
    ClearScores(show_score);
  ...
  void ShowScore(
    const ausim_msg::msg::Detection3D detection, const double score,
    const size_t idx)
  {
    auto marker = std::make_shared<Marker>();
